...
import sys
# sys.path.append('/home/lamberti/work/tii/dataset_browser')
# from dataset_browser.dataset_processing import match_images_and_states
...
class Acquisition:
    ...
    def labels_from_csv(self, filename):
        ...
        for label in labels_per_image:
            self.images.append(
                Image(
                    os.path.join(self.images_dir_path, label['filename']),
                    {
                        label[len(self.LABEL_CSV_PREFIX):]: value
                        for label, value in label.items()
                        if label.startswith(self.LABEL_CSV_PREFIX)
                    },
                    label['partition'] if ('partition' in label) else None
                )
            )
